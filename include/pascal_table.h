...
namespace pascals {
    ...
    public:
        ...
        std::shared_ptr<std::unordered_map<std::string, std::shared_ptr<Identifier>>> get_map() {  // 获取当前符号表
            return std::make_shared<std::unordered_map<std::string, std::shared_ptr<Identifier>>>(m_map); 
        }
        std::shared_ptr<SymbolTable> get_prev_table() {  // 获取上一层符号表
            return m_prev_table; 
        }
        bool insert_element(std::string name, std::shared_ptr<Identifier> identifier_ptr) {  // 向表中添加新的键值对
        ...
        std::shared_ptr<Identifier> get_identifier_by_name(std::string name) {  // 根据标识符名返回对应标识符的指针
    ...
    protected:
        ...
        std::unordered_map<std::string, std::shared_ptr<Identifier>> m_map;  // 键为标识符名，值为对应该标识符的指针
        std::shared_ptr<SymbolTable> m_prev_table;  // 指向外层符号表的指针
    };
}
