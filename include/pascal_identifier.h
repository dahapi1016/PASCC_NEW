...
namespace pascals {
	...
	public:
		...
		Identifier();
		Identifier(std::string name, TypeTemplate* value_type, bool is_reference = false, int declare_line = 0);  // 默认构造为变量名
		virtual ~Identifier();
		std::string get_name();  // 获取标识符名
		TypeTemplate* get_value_type();  // 获取标识符值的类型
	...
	protected:
		...
		TypeTemplate* m_value_type;  // 标识符值的类型
	...
	public:
		struct Parameter {  // 函数参数
			...
			TypeTemplate* param_type;  // 参数类型
			bool is_reference;   // 是否引用类型
		};

		FunctionIdentifier();
		FunctionIdentifier(const FunctionIdentifier& other);  // 深拷贝构造
		FunctionIdentifier(std::string name, BasicType* return_type, int decl_line, const std::vector<Parameter>& params_list);
		...
		TypeTemplate* get_param_type_by_name(std::string name);  // 根据参数名获取其类型
		bool get_param_is_reference_by_name(std::string name);  // 根据参数名获取是否为引用
