...
namespace pascals {
	...
	public:
		...
		static bool is_basic_type(TypeTemplate* type);  // 判t是否为基础类型
		bool is_same(TypeTemplate* type1, TypeTemplate* type2);  // 判两个类型是否相同
	protected:
		TYPE m_type;  // 类型
	};

	class BasicType : public TypeTemplate {  // 继承自TypeTemplate，表示基础类型
	public:
		...
		BasicType* get_computed_type(BasicType* type1, BasicType* type2, std::string op);  // 两种类型进行op操作后的结果类型
		BasicType* get_computed_type(BasicType* type1, std::string op);  // 一种类型进行op操作后的结果类型
		bool is_assignable_from(BasicType* other) {
			if (m_basic_type == BASIC_TYPE::REAL && other->m_basic_type == BASIC_TYPE::INTEGER)
				return true;  // 允许 real ← int
			return m_basic_type == other->m_basic_type;
		}
	private:
		BASIC_TYPE m_basic_type;  // 基础类型
	};

	class ArrayType : public TypeTemplate {  // 继承自TypeTemplate，表示数组类型
	public:
		...
		ArrayType();  // 无参构造
		ArrayType(BasicType* element_type);  // 一个参数(元素类型)构造
		ArrayType(BasicType* element_type, std::vector<ArrayBound> bounds);  // 两个参数(元素类型、数组上下界)构造
		...
		std::string get_pascal_name() override;  // 获取对应的Pascal风格数据类型名称
		BasicType* get_element_type();  // 获取数组元素类型
	...
	private:
		BasicType* m_element_type;  // 元素类型
		std::vector<ArrayBound> m_bounds;  // 数组各维数上下界(多维)
	};

	// 全局变量
	extern BasicType* TYPE_INTEGER;
	extern BasicType* TYPE_REAL;
	extern BasicType* TYPE_BOOLEAN;
	...
	private:
		BasicType* m_const_type = nullptr;  // 常量类型
		std::variant<int, float, bool, char, std::string> m_const_value;  // 常量值
	};

	class Operation {  // 运算类
	public:
		Operation(BasicType* operand1_type, BasicType* operand2_type, const std::string& op);
		~Operation();
		bool operator==(const Operation& other) const;  // 判等操作
		BasicType* m_operand1_type;  // 操作数1类型
		BasicType* m_operand2_type;  // 操作数2类型(若为一元运算符则为null)
		std::string m_op;  // 运算符
	};
	...
	extern std::unordered_map<Operation, BasicType*, OperationHash> g_operation_map;  // 键为运算类，值为运算后结果类型

	void pascal_types_init();  // 全局变量初始化
	void pascal_types_cleanup();  // 全局变量清理
}